...
def inside_hotspot(
    ...
    for location in hotspots:
        ...
        if polygon.contains(user_point):
            ...
            other_users_id = db.execute(text(f"SELECT user_id from {location.name}_temporary_table WHERE user_id != {current_user.id};")).fetchall()
            ids=[]
            for user_id in ids:
             other_users = oauth2.get_user_by_id(user_id[0], db)
             other_profiles = oauth2.get_user_profile_by_id(user_id[0], db)
             return {
                    "inside": True,
                    "user": {
                        "id": other_users.id,
